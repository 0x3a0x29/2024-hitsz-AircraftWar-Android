...
import com.example.aircraftwar2024.music.BossMediaPlayer;
...
public abstract class BaseGame extends SurfaceView implements SurfaceHolder.Callback, Runnable{
    ...
    private BossMediaPlayer bossMediaPlayer;
    ...
    protected boolean openMusic = false;
    /**
     * 时间间隔(ms)，控制刷新频率
     */
    ...
    public BaseGame(Context context,Handler mHandler,boolean music){
        super(context);
        ...
        bossMediaPlayer = new BossMediaPlayer(context);
    ...
    protected List<AbstractEnemyAircraft> produceBoss() {
        ...
        if (this.getScore() >= bossScoreThreshold && !this.existBoss()) {
            ...
            if(openMusic){
                ...
                bossMediaPlayer.loopRunBossMusic();
            }
        }
        return res;
    }
    ...
    private void crashCheckAction() throws InterruptedException {
            ...
            for (AbstractEnemyAircraft enemyAircraft : enemyAircrafts) {
                ...
                if (enemyAircraft.crash(bullet)) {
                    ...
                    if (enemyAircraft.notValid()) {
                        ...
                        if(openMusic){
                            mySoundPool.startBulletHitSound();
                        }

                        if(enemyAircraft instanceof BossEnemy && openMusic){
                            bossMediaPlayer.stopBossMusic();
        ...
        for (AbstractFlyingSupply flyingSupply : flyingSupplies) {
            ...
            if (heroAircraft.crash(flyingSupply) || flyingSupply.crash(heroAircraft)) {
                ...
                if(flyingSupply instanceof BombSupply && openMusic){
                    mySoundPool.startBombExplosion();
                }
                else if(openMusic){
                    mySoundPool.startGetSupplySound();
                }
    ...
    protected void gameOver(){
        ...
        ScoreData scoreData=new ScoreData("test",score,new Date());
    ...
    protected void paintScoreAndLife() {
        ...
        canvas.drawText("SCORE: "+Integer.toString(this.score),20,80,mPaint);
        canvas.drawText("LIFE: "+Integer.toString(heroAircraft.getHp()),20,160,mPaint);
    }
    protected void paintGameOver() {
        if(gameOverFlag){
            ...
            canvas.drawText("YOU ARE DEAD", 100, 1000, mPaint);
        }
    }
    @Override
